<?php
/**
 * Template part for Partner's bottom navigation.
 *
 *
 * @package txakolina
 */

?>
<div class="partner-bottom-nav">
    <div class="partner-bottom-nav-header-wrapper">
        <h3 class="partner-bottom-nav-title">Bottom <span>Navigation</span> Header</h3>
        <div class="divider"></div>
    </div>
    <p class="partner-bottom-nav-snip">Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Lorem ipsum dolor sit amet, consectetur adipisicing elit.</p>
    <div class="partner-bottom-nav-wrapper">
        <div class="partner-bottom-nav-element">
            <div class="partner-bottom-nav-element-inner-wrapper">
                <img src="http://vccw.test/wp-content/uploads/2018/02/@.png" alt="">
                <h4>About NCP</h4>
                <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>
            </div>
        </div>
        <div class="partner-bottom-nav-element">
            <div class="partner-bottom-nav-element-inner-wrapper">
                <img src="http://vccw.test/wp-content/uploads/2018/02/@.png" alt="">
                <h4>STARTING YOUR OWN CATH LAB </h4>
                <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>
            </div>
        </div>
        <div class="partner-bottom-nav-element">
            <div class="partner-bottom-nav-element-inner-wrapper">
                <img src="http://vccw.test/wp-content/uploads/2018/02/@.png" alt="">
                <h4>LAB FACILITIES</h4>
                <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>
            </div>
        </div>
        <div class="partner-bottom-nav-element">
            <div class="partner-bottom-nav-element-inner-wrapper">
                <img src="http://vccw.test/wp-content/uploads/2018/02/@.png" alt="">
                <h4>Careers</h4>
                <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>
            </div>
        </div>
    </div>
</div>
<div class="partner-social">
    <div class="partner-social-inner-wrapper">
        <h3 class="partner-social-cta-text">Follow<br/>National Cardiovascular Partners</h3>
        <div class="partner-social-icon-wrapper">
            <img class="partner-social-icon-left" style="width:57px;height:57px;" src="http://vccw.test/wp-content/uploads/2018/02/Twitter-logo-1.png" alt="Twitter Logo">
            <img class="partner-social-icon-right" src="http://vccw.test/wp-content/uploads/2018/02/linkedIn.png" alt=""><!-- https://fmcnaqa1.wpengine.com/wp-content/uploads/2018/01/linkedIn-logo.png -->
        </div>
    </div>
</div>
<div class="social-share">
    <div class="social-share-inner-wrapper">
        <div class="social-share-text">
            Share:
        </div>
        <div class="social-share-icon-wrapper">
            <div class="social-share-icon-item">
            <img src="http://vccw.test/wp-content/uploads/2018/02/social_share.png" alt="Social Icons"/>    
            <!--<img src="http://placecorgi.com/40/40" alt="">
                <span class="divider">|</span>
                <div class="social-share-print-wrapper">
                    <div class="print-icon">
                        <img src="http://placecorgi.com/40/40" >
                    </div>
                    <div class="print-text">
                        Print
                    </div>
                </div>-->
            </div>
        </div>
    </div>
</div>
<!--<div class="legal-section">
    <div class="legal-section-inner-wrapper">
        <h3>Legal Section</h3>
        <p>A posuere scelerisque gravida facilisi parturient magna urna donec parturient per augue a amet quam nullam a ad a. Parturient velit imperdiet in suspendisse.</p>
    </div>
</div>-->